...
for i in range(len(PE)):
    low_PE[i] = membership.decreasing(PE[i], 0, 10)
    med_PE[i] = membership.trape(PE[i], 8, 20, 20, 30)
    high_PE[i] = membership.increasing(PE[i], 20, 30)
...
for i in range(len(ROE)):
    poor_ROE[i] = membership.decreasing(ROE[i], 5, 7)
    ave_ROE[i] = membership.trape(ROE[i], 5, 9, 9, 13)
    high_ROE[i] = membership.increasing(ROE[i], 10, 15)
...
for i in range(len(GPP)):
    Neg_GPP[i] = membership.gauss1(GPP[i], -1, 0.25)
    Neu_GPP[i] = membership.gauss(GPP[i], 0, 0.16)
    Pos_GPP[i] = membership.gauss2(GPP[i], 1, 0.25)
...
for i in range(len(MAJEC)):
    Dec_MAJEC[i] = membership.decreasing(MAJEC[i], -1, 0)
    Const_MAJEC[i] = membership.trape(MAJEC[i], -1, 0, 0, 1)
    Inc_MAJEC[i] = membership.increasing(MAJEC[i], 0, 1)
...
for i in range(len(SPM)):
    Red_SPM[i] = membership.gauss1(SPM[i], -5, 0.6)
    Const_SPM[i] = membership.gauss(SPM[i], 0, 1.9)
    Inc_SPM[i] = membership.gauss2(SPM[i], 5, 0.6)
